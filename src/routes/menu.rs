...
use crate::routes::common::{ApiResult, data_response, error_response, get_string, get_bool, get_i64, get_array, document_id};
...
async fn get_menu(Path(shop_id): Path<String>, State(db): State<Database>) -> ApiResult{
    ...
    while let Some(doc) = cursor.try_next()
        ...
        let price = get_i64(&doc, "price");
        let sizes = get_array(&doc, "sizes");
        ...
        item.insert("price", match price { Some(v) => Bson::Int64(v), None => Bson::Null });
    ...
    Ok(data_response(Bson::Document(doc! { "items": Bson::Array(results) })))
}
